...
describe('Footer tests', () => {
  describe('SystemInfo returns content', () => {
    ...
    test('renders correctly with system info content', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('footer-see-source-code')).toBeInTheDocument();
      });
      ...
      expect(footer_see_source_code).toHaveTextContent('See the source code on Github.');
  ...
  describe('SystemInfo is returning null', () => {
    ...
    test('renders correctly without system info content', async () => {
      ...
      await waitFor(() => {
        const expectedText = 'This is a sample webapp using React with a Spring Boot backend.';
        expect(screen.getByTestId('Footer').textContent).toBe(expectedText);
      });
